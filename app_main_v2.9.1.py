...
"""
RuundPDF v2.9.1 - PDF Reader с - это продакшн!
РАБОЧИМ управлением озвучкой и ВОССТАНОВЛЕННЫМ копированием текста
Все кнопки работают правильно, восстановлено копирование текста и правильное чтение
...
# ============================================================================
# КЛАСС ДЛЯ УПРАВЛЕНИЯ ОЗВУЧКОЙ (ПЕРЕПИСАННЫЙ ДЛЯ УПРАВЛЕНИЯ)
# ============================================================================
class TTSController(QObject):
    """Контроллер для управления озвучкой с правильным управлением"""
    ...
    def start_reading(self, start_page=0, read_all=True):
        """Начинает чтение с указанной страницы"""
        if self.is_running:
            return
        ...
        if read_all:
            ...
            self.end_page = start_page  # Только текущая страница
        
        # Запускаем поток чтения
        self.reading_thread = threading.Thread(target=self._read_document)
    ...
    def pause_reading(self):
        ...
        if self.is_running and not self.is_paused:
            ...
            # Останавливаем текущий TTS движок
    ...
    def _read_document(self):
        """Основная функция чтения документа"""
        try:
            # Читаем страницы
            for page_num in range(self.current_page, self.end_page + 1):
                ...
                try:
                    # Создаем новый движок для страницы
                    self.current_tts_engine = pyttsx3.init()
                    self._setup_voice(self.current_tts_engine)
                    
                    # ГОВОРИМ ВЕСЬ ТЕКСТ СТРАНИЦЫ СРАЗУ, БЕЗ РАЗБИВКИ НА ПРЕДЛОЖЕНИЯ
                    self.current_tts_engine.say(text)
                    self.current_tts_engine.runAndWait()
                    
                except Exception as e:
                    print(f"Ошибка озвучивания страницы {page_num}: {e}")
                    continue
                ...
                if not self.stop_event.is_set() and not self.should_stop:
                    time.sleep(0.1)
...
class TTSPlayerWidget(QWidget):
    ...
    def setup_ui(self):
        ...
        # Текущая страница
        self.page_label = QLabel(f"Страница: {self.start_page + 1} из {self.total_pages}")
        main_layout.addWidget(self.page_label)
        
        # Прогресс-бар
        self.progress_bar = QProgressBar()
        self.progress_bar.setRange(0, self.total_pages)
        self.progress_bar.setValue(self.start_page)
        ...
        self.radio_current = QRadioButton(f"Текущая страница ({self.start_page + 1})")
        self.radio_from_current = QRadioButton(f"С текущей страницы до конца")
        self.radio_all = QRadioButton("Весь документ (с начала)")
        
        self.radio_from_current.setChecked(True)
        
        mode_layout.addWidget(self.radio_current)
        mode_layout.addWidget(self.radio_from_current)
        ...
        self.btn_settings = QPushButton("⚙ Настройки голоса")
    ...
    def update_buttons(self):
        ...
        self.radio_from_current.setEnabled(not is_running)
        self.radio_all.setEnabled(not is_running)
    
    def get_reading_mode(self):
        """Определяет режим чтения"""
        if self.radio_current.isChecked():
            return 'current_only'  # Только текущая страница
        elif self.radio_from_current.isChecked():
            return 'from_current'  # С текущей до конца
        else:  # radio_all
            return 'all'  # Весь документ
    ...
    def start_reading(self):
        ...
        if mode == 'current_only':
            ...
            read_all = False
            self.reading_info.setText(f"Читается только страница {start_page + 1}")
        elif mode == 'from_current':
            start_page = self.start_page
            read_all = True
            self.reading_info.setText(f"Чтение с {start_page + 1} страницы до конца")
        else:  # 'all'
            start_page = 0
            read_all = True
            self.reading_info.setText("Чтение всего документа с начала")
        
        self.controller.start_reading(start_page, read_all)
    ...
    def stop_reading(self):
        ...
        # Сбрасываем прогресс на начальную страницу
        self.progress_bar.setValue(self.start_page)
        self.page_label.setText(f"Страница: {self.start_page + 1} из {self.total_pages}")
    ...
    def on_progress(self, page_num):
        ...
        self.page_label.setText(f"Страница: {page_num + 1} из {self.total_pages}")
    ...
    def on_paused(self):
        ...
        self.reading_info.setText(f"Пауза на странице {self.controller.current_page + 1}")
        self.update_buttons()
    ...
    def on_resumed(self):
        ...
        self.status_label.setText("Чтение...")
        self.status_label.setStyleSheet("color: green; font-weight: bold; font-size: 12pt;")
        self.reading_info.setText(f"Продолжение чтения со страницы {self.controller.current_page + 1}")
        self.update_buttons()
    ...
    def show_settings(self):
        """Показывает настройки голоса"""
        dialog = QDialog(self)
        dialog.setWindowTitle("Настройки голоса")
        ...
        layout.addWidget(self.radio_male)
        layout.addWidget(self.radio_female)
        
        btn_layout = QHBoxLayout()
        btn_ok = QPushButton("OK")
        btn_cancel = QPushButton("Отмена")
        
        btn_ok.clicked.connect(lambda: self.save_voice_settings(dialog))
...
class PDFViewerApp(QMainWindow):
    ...
    def show_about_dialog(self):
        ...
        info_text.setHtml("""
            <p align='center'><strong>RuundPDF v2.9.1</strong></p>
            <p align='center'>© DeeR Tuund 2025</p>
            <p>PDF ридер с ПОЛНОСТЬЮ РАБОЧЕЙ озвучкой и поиском</p>
            <p>Функции:</p>
            <ul>
                <li>Чтение PDF файлов</li>
                <li>Озвучка текста (весь документ или отдельные страницы)</li>
                <li>Пауза/Продолжение/Остановка чтения</li>
                <li>Поиск с подсветкой результатов</li>
                <li>Закладки</li>
                <li>Drag-and-Drop</li>
                <li>Печать</li>
                <li>ВОССТАНОВЛЕННОЕ копирование текста</li>
            </ul>
        """)
