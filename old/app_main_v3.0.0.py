...
class TTSController(QObject):
    ...
    def _setup_voice(self, tts_engine):
        """Настраивает голос TTS с учетом языка текста - ИСПРАВЛЕННАЯ ВЕРСИЯ"""
        ...
        try:
            ...
            print(f"Текст для анализа (первые 100 символов): {text[:100]}")
            
            # Определяем язык, если включена опция
            target_language = None
            if auto_lang and text and text.strip():
                try:
                    detected = detect(text[:500])  # Анализируем первые 500 символов
                    target_language = detected
                    print(f"Определен язык текста: {target_language}")
                except Exception as e:
                    print(f"Ошибка определения языка: {e}")
                    target_language = 'en'  # Язык по умолчанию при ошибке
            ...
            for voice in voices:
                ...
                # 1. Если определен язык - ищем по языку и полу
                if target_language:
                    lang_match = False
                    gender_match = False
                    
                    # Ключевые слова для английского
                    if target_language == 'en':
                        lang_keywords = ['english', 'en-', 'en_', 'us', 'uk', 'gb', 'english']
                    # Ключевые слова для русского
                    elif target_language == 'ru':
                        lang_keywords = ['russian', 'ru-', 'ru_', 'rus']
                    else:
                        # Для других языков берем первые 2 символа кода языка
                        lang_prefix = target_language[:2]
                        lang_keywords = [lang_prefix + '-', lang_prefix + '_', lang_prefix]
                    
                    # Проверяем совпадение по языку
                    for keyword in lang_keywords:
                        if keyword in voice_id or keyword in voice_name:
                            lang_match = True
                            break
                    
                    # Проверяем совпадение по полу
                    if use_female:
                        gender_keywords = ['female', 'женск', 'женский', 'woman']
                    else:
                        gender_keywords = ['male', 'мужск', 'мужской', 'man']
                    ...
                    if lang_match and gender_match:
                        ...
                        print(f"Найден подходящий голос по языку и полу: {voice.name}")
                        break
                
                # 2. Если не нашли по языку, ищем только по полу
                if not selected_voice:
                    ...
                    if gender_match:
                        selected_voice = voice.id
                        print(f"Найден голос только по полу (язык не определен/не найден): {voice.name}")
                        break
            
            # 3. Если ничего не нашли, используем fallback
            if not selected_voice and fallback_voice:
                selected_voice = fallback_voice
                print(f"Используется голос по умолчанию: {voices[0].name}")
            
            # Устанавливаем голос с обработкой ошибок
            if selected_voice:
                try:
                    tts_engine.setProperty('voice', selected_voice)
                    print(f"Установлен голос ID: {selected_voice}")
                    
                    # ТЕСТ: попробуем сказать короткую английскую фразу
                    test_text = "Test"
                    print(f"Пробуем протестировать голос с текстом: '{test_text}'")
                    
                except Exception as e:
                    print(f"КРИТИЧЕСКАЯ ОШИБКА при установке голоса: {e}")
                    print(f"ID голоса который вызвал ошибку: {selected_voice}")
                    
                    # Пробуем другие голоса
                    for i, voice in enumerate(voices[:5]):  # Пробуем первые 5 голосов
                        try:
                            tts_engine.setProperty('voice', voice.id)
                            print(f"Альтернативный голос установлен: {voice.name}")
                            break
                        except:
                            continue
            
            # Логируем все доступные голоса для диагностики
            print("\n=== ДИАГНОСТИКА ГОЛОСОВ ===")
            ...
            print(f"Общая ошибка в _setup_voice: {e}")
...
class PDFViewerApp(QMainWindow):
    ...
    def show_about_dialog(self):
        ...
        info_text.setHtml("""
            <p align='center'><strong>RuundPDF v3.0.0</strong></p>
