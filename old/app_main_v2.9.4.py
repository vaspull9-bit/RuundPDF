"""
RuundPDF v2.9.4 - PDF Reader с РАБОЧИМ управлением озвучкой и ВОССТАНОВЛЕННЫМ копированием текста
С РАБОЧЕЙ кнопкой Пауза, убранными дубликатами и чтением в цикле
...
# ============================================================================
# КЛАСС ДЛЯ УПРАВЛЕНИЯ ОЗВУЧКОЙ (УПРОЩЕННЫЙ И РАБОЧИЙ)
# ============================================================================
class TTSController(QObject):
    """Контроллер для управления озвучкой с ПРОСТОЙ и РАБОЧЕЙ паузой"""
    ...
    def pause_reading(self):
        """Ставит чтение на паузу - ПРОСТОЙ и РАБОЧИЙ МЕТОД"""
        if self.is_running and not self.is_paused:
            self.is_paused = True
            
            # НЕМЕДЛЕННО останавливаем TTS движок
            if self.tts_engine:
                try:
                    # ЭТО КЛЮЧЕВОЙ МОМЕНТ - полностью останавливаем движок
            ...
            self.paused.emit()
            print("Чтение ПАУЗА - движок остановлен")
            ...
            while not self.should_stop:
                ...
                for page_num in range(self.current_page, self.read_to_page + 1):
                    ...
                    try:
                        ...
                        # Проверяем паузу после чтения страницы
                        if self.pause_event.is_set():
                            # Ждем снятия паузы
                            self.resume_event.wait()
                            self.resume_event.clear()
...
# ============================================================================
# КЛАСС НАСТРОЕК ПЛЕЕРА (ВСЕ НАСТРОЙКИ В ОДНОМ МЕСТЕ)
# ============================================================================
...
# ============================================================================
# КЛАСС ПЛЕЕРА ОЗВУЧКИ (УПРОЩЕННЫЙ, БЕЗ ДУБЛИРУЮЩИХ НАСТРОЕК)
# ============================================================================
class TTSPlayerWidget(QWidget):
    ...
    def pause_reading(self):
        """Ставит чтение на паузу - ПРОСТО И ЭФФЕКТИВНО"""
...
class PDFViewerApp(QMainWindow):
    ...
    def show_about_dialog(self):
        ...
        info_text.setHtml("""
            <p align='center'><strong>RuundPDF v2.9.4</strong></p>
            ...
            <ul>
                ...
                <li>Отслеживание текущей страницы</li>
                <li>Поиск с подсветкой результатов</li>
                <li>Закладки, копирование текста, печать</li>
            </ul>
        """)
