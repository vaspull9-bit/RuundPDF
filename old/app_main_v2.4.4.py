...
"""
RuundPDF v2.4.5 - PDF Reader with TTS and advanced features
ИСПРАВЛЕННЫЙ ВАРИАНТ - Все ошибки устранены
...
# ============================================================================
# 2. ФУНКЦИЯ СОЗДАНИЯ ИКОНОК ИЗ ТЕКСТА
# ============================================================================
...
def register_file_association():
    ...
    try:
        ...
        print(f"Ассоциация файлов зарегистрирована")
        return True
...
class AboutDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        info_text.setHtml("""
            <p align='center'><strong>RuundPDF v2.4.5</strong></p>
            <p align='center'>© DeeR Tuund 2025</p>
            <p>Возможности:</p>
            <ul>
                <li>Чтение PDF файлов</li>
                <li>Выделение и копирование текста</li>
                <li>Озвучивание текста (плеер с паузой/стопом)</li>
                <li>Зум, поворот страницы</li>
                <li>Печать, сохранение копии</li>
                <li>Drag-and-Drop загрузка</li>
                <li>Система закладок</li>
                <li>Ассоциация с PDF файлами</li>
            </ul>
        """)
...
# ============================================================================
# 5. КЛАСС НАСТРОЕК ПЛЕЕРА (ИСПРАВЛЕННЫЙ)
# ============================================================================
class TTSConfigDialog(QDialog):
    """Диалог настроек плеера с РАБОЧЕЙ логикой."""
    ...
    def setup_ui(self):
        ...
        self.page_num_edit.setMaximumWidth(50)
    ...
    def apply_settings(self):
        """ПРАВИЛЬНО применяет все настройки."""
        ...
        if self.radio_start.isChecked():
            ...
            self.player.current_read_page = 0
            ...
            try:
                ...
                if 0 <= page < self.player.total_pages:
                    ...
                    self.player.current_read_page = page
                else:
                    QMessageBox.warning(self, "Ошибка", "Номер страницы вне диапазона.")
                    return
            except ValueError:
                QMessageBox.warning(self, "Ошибка", "Введите корректный номер страницы.")
                return
...
# ============================================================================
# 6. КЛАСС ПЛЕЕРА (ПОЛНОСТЬЮ ПЕРЕПИСАННЫЙ)
# ============================================================================
class TTSPlayerWidget(QWidget):
    """Плеер озвучки с РАБОЧЕЙ логикой чтения."""
    ...
    def load_settings(self):
        self.use_male = self.settings.value("tts_use_male", True, type=bool)
        self.use_female = self.settings.value("tts_use_female", False, type=bool)
    ...
    def apply_voice_settings(self):
        ...
        if self.use_female:
            ...
            for voice in voices:
                voice_lower = voice.name.lower()
                if 'male' in voice_lower or 'мужск' in voice_lower or 'russian' in voice_lower:
                    self.tts_engine.setProperty('voice', voice.id)
                    return
    ...
    def playback_worker(self):
        """РАБОЧИЙ поток воспроизведения - читает ВСЕ страницы правильно."""
        try:
            # Определяем с какой страницы начинать
            if self.read_mode == 'start':
                current_page = 0
            elif self.read_mode == 'specific':
                current_page = self.specific_page
            else:
                current_page = self.current_read_page
            
            # Определяем сколько страниц читать
            if self.read_mode == 'only_current':
                # Только одну страницу
                pages_to_read = [current_page]
            else:
                # От текущей до конца
                pages_to_read = list(range(current_page, self.total_pages))
            ...
            for page_num in pages_to_read:
                ...
                if text:
                    # Обновляем текущую страницу для отображения
                    ...
                    try:
                        ...
                        print(f"Ошибка озвучивания стр. {page_num}: {e}")
                        break
                else:
                    break
...
# ============================================================================
# 7. КАСТОМНЫЙ QGraphicsView (С РАБОЧИМ DRAG-AND-DROP)
# ============================================================================
class PDFGraphicsView(QGraphicsView):
    """Кастомный QGraphicsView с РАБОЧИМ drag-and-drop."""
    ...
    def dragEnterEvent(self, event):
        """Принимаем PDF файлы при drag-and-drop."""
        if event.mimeData().hasUrls():
            urls = event.mimeData().urls()
            if urls and urls[0].toLocalFile().lower().endswith('.pdf'):
                event.acceptProposedAction()
                self.setStyleSheet("QGraphicsView { background-color: #e0f0ff; border: 2px dashed #4A90E2; }")
    
    def dragLeaveEvent(self, event):
        """Сбрасываем стиль при уходе drag-and-drop."""
        self.setStyleSheet("QGraphicsView { background-color: #fff; border: 1px solid #ccc; }")
    ...
    def dropEvent(self, event):
        ...
        if urls:
            ...
            if file_path.lower().endswith('.pdf'):
                ...
                QMessageBox.warning(self.main_app, "Ошибка", "Поддерживаются только PDF файлы")
                event.ignore()
        
        self.setStyleSheet("QGraphicsView { background-color: #fff; border: 1px solid #ccc; }")
    
    def wheelEvent(self, event):
        """Обработчик колеса мыши - Ctrl+Колесо для зума."""
...
# ============================================================================
# 8. ОСНОВНОЙ КЛАСС ПРИЛОЖЕНИЯ (ВСЕ ИСПРАВЛЕНИЯ)
# ============================================================================
class PDFViewerApp(QMainWindow):
            ...
            if first_run:
                if register_file_association():
                    QMessageBox.information(self, "Ассоциация файлов", 
                                          "Программа зарегистрирована для открытия PDF файлов.")
    ...
    def setup_ui(self):
        ...
        self.action_speak = QAction(create_text_icon("📖"), "Открыть плеер", self)
        ...
        self.action_bookmark.triggered.connect(self.manage_bookmarks)  # Прямой вызов диалога
        toolbar.addAction(self.action_bookmark)
        
        self.action_toggle_cursor = QAction(create_text_icon("👆"), "Переключить курсор", self)
        self.action_toggle_cursor.triggered.connect(self.toggle_cursor_mode)
        toolbar.addAction(self.action_toggle_cursor)
        
        self.action_copy_text = QAction(create_text_icon("📋"), "Копировать текст", self)
        ...
        self.status_bar.showMessage("Готово. Перетащите PDF файл в окно или откройте через меню.")
    ...
    def dropEvent(self, event: QDropEvent):
        ...
        if urls:
            ...
            if file_path.lower().endswith('.pdf'):
                ...
                QMessageBox.warning(self, "Ошибка", "Поддерживаются только PDF файлы")
                event.ignore()
        
        self.apply_styles()
    
    def keyPressEvent(self, event):
        """Обработчик клавиш с ЦИКЛИЧЕСКИМ переходом по страницам."""
        if event.key() == Qt.Key.Key_PageDown:
            if self.document:
                if self.current_page_num < self.document.page_count - 1:
                    self.current_page_num += 1
                else:
                    self.current_page_num = 0  # Цикл: после последней - первая
                self.render_page()
            event.accept()
            ...
            if self.document:
                if self.current_page_num > 0:
                    ...
                    self.current_page_num = self.document.page_count - 1  # Цикл: перед первой - последняя
                self.render_page()
            event.accept()
                ...
                if "lines" in block:
                    for line in block["lines"]:
                        for span in line["spans"]:
                            ...
                            text_blocks.append({'text': text, 'bbox': scaled_bbox, 'rect': rect})
        except Exception as e:
    ...
    def save_file_as(self, default_path=""):
        ...
        file_path, _ = QFileDialog.getSaveFileName(self, "Сохранить как", default_path, "PDF Files (*.pdf)")
        if file_path:
            try:
                self.document.save(file_path)
                self.file_path = file_path
                QMessageBox.information(self, "Успех", f"Файл сохранен как {QFileInfo(file_path).fileName()}.")
    ...
    def add_bookmark(self):
        ...
        if existing_name:
            reply = QMessageBox.question(self, 'Закладка', 
                                        f"Закладка '{existing_name}' уже существует.\nУдалить её?",
    ...
    def manage_bookmarks(self):
        """Диалог управления закладками с РАБОЧИМ удалением."""
        dialog = QDialog(self)
        ...
        if (self.is_text_select_mode and 
            ...
            if self.selection_rect.width() > 5 and self.selection_rect.height() > 5:
                ...
                if self.selected_text:
                    ...
                    self.status_bar.showMessage(f"Текст выделен: {char_count} символов. Ctrl+C для копирования")
                else:
                    self.clear_selection()
            else:
                self.clear_selection()
    ...
    def copy_selected_text_to_clipboard(self):
        if self.selected_text:
            ...
            char_count = len(self.selected_text)
            self.status_bar.showMessage(f"Текст скопирован: {char_count} символов")
            QTimer.singleShot(1000, self.clear_selection)
        else:
            self.status_bar.showMessage("Нет выделенного текста")
