...
class AboutDialog(QDialog):
    def __init__(self, parent=None):
        ...
        info_text.setHtml("""
            <p align='center'><strong>Программа RuundPDF v2.4.2</strong></p>
...
class TTSPlayerWidget(QWidget):
    ...
    def apply_voice_settings(self):
        ...
        if self.use_female:
            ...
            # Ищем мужской голос
            for voice in voices:
                if 'male' in voice.name.lower() or 'мужск' in voice.name.lower() or 'russian' in voice.name.lower():
                    self.tts_engine.setProperty('voice', voice.id)
                    return
