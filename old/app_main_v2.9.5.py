"""
RuundPDF v2.9.5 - PDF Reader с РАБОЧИМ управлением озвучкой и АКТИВНЫМИ ссылками
...
from PyQt6.QtGui import QDesktopServices  # ДОБАВЛЕНО!
...
# ============================================================================
# КЛАСС ДЛЯ УПРАВЛЕНИЯ ОЗВУЧКОЙ (ПЕРЕРАБОТАННЫЙ - ФИКС ПАУЗЫ)
# ============================================================================
class TTSController(QObject):
    """Контроллер для управления озвучкой с ИСПРАВЛЕННОЙ паузой"""
    ...
    def __init__(self, text_provider, total_pages):
        ...
        # Для хранения состояния при паузе
    ...
    def start_reading(self):
        ...
        self.tts_thread = threading.Thread(target=self._read_document_thread)
    ...
    def pause_reading(self):
        """Ставит чтение на паузу - ИСПРАВЛЕННЫЙ МЕТОД"""
        if self.is_running and not self.is_paused:
            self.is_paused = True
            self.pause_flag.set()
            
            # Останавливаем движок pyttsx3
            if self.tts_engine:
                try:
    ...
    def resume_reading(self):
        """Продолжает чтение с паузы - ИСПРАВЛЕННЫЙ МЕТОД"""
    ...
    def stop_reading(self):
        ...
        if self.is_running:
            ...
            # Устанавливаем все флаги для выхода
            self.stop_flag.set()
            self.pause_flag.set()
            self.resume_flag.set()
    ...
    def _read_document_thread(self):
        """Основная функция чтения документа в потоке"""
        try:
            # Инициализируем TTS движок ОДИН раз
            self.tts_engine = pyttsx3.init()
            self._setup_voice(self.tts_engine)
            
            # Список страниц для чтения
            pages_to_read = list(range(self.read_from_page, self.read_to_page + 1))
            
            while not self.should_stop:
                for i, page_num in enumerate(pages_to_read):
                    if self.should_stop or self.stop_flag.is_set():
                        break
                    
                    # Проверяем паузу перед началом страницы
                    if self.pause_flag.is_set():
                        print(f"Пауза на странице {page_num}")
                        self.resume_flag.wait()  # Ждем снятия паузы
                        self.resume_flag.clear()
                    ...
                    page_text = self.text_provider(page_num)
                    ...
                    # Читаем каждое предложение
                    for sentence_idx, sentence in enumerate(sentences):
                        if self.should_stop or self.stop_flag.is_set():
                            break
                        
                        # Проверяем паузу перед каждым предложением
                        if self.pause_flag.is_set():
                            print(f"Пауза внутри страницы {page_num}, предложение {sentence_idx}")
                            self.resume_flag.wait()  # Ждем снятия паузы
                            self.resume_flag.clear()
                        ...
                        if sentence.strip():
                            try:
                                # Очищаем очередь и читаем предложение
                                self.tts_engine.stop()
                    ...
                    if not self.should_stop and i < len(pages_to_read) - 1:
                        time.sleep(0.2)
    ...
    def _split_into_sentences(self, text):
        """Разбивает текст на предложения для лучшего контроля"""
        if not text:
            return []
        ...
        # Если не нашли разделителей, возвращаем весь текст как одно предложение
        if not sentences:
...
class PDFGraphicsView(QGraphicsView):
    ...
    def paintEvent(self, event):
        """
        ВОССТАНОВЛЕННЫЙ метод из v2.4.4 - отрисовка выделения текста и активных ссылок
        """
        ...
        # Отрисовка активных ссылок (полупрозрачные прямоугольники)
        for link in self.clickable_links:
            # Преобразуем координаты сцены в координаты вида
            view_rect = self.mapFromScene(link['rect']).boundingRect()
            
            painter.setPen(QPen(QColor(0, 100, 200, 150), 1, Qt.PenStyle.DashLine))
            painter.setBrush(QBrush(QColor(0, 100, 200, 30)))
            painter.drawRect(view_rect)
            
            # Подпись для ссылки (только в режиме отладки)
            if hasattr(self.main_app, 'debug_mode') and self.main_app.debug_mode:
                painter.setPen(Qt.GlobalColor.darkBlue)
                link_preview = link['uri'][:30] + "..." if len(link['uri']) > 30 else link['uri']
                painter.drawText(view_rect.bottomLeft() + QPointF(5, 15), f"Ссылка: {link_preview}")
...
class PDFViewerApp(QMainWindow):
    ...
    def __init__(self, file_to_open=None):
        super().__init__()
        self.setWindowTitle("RuundPDF v2.9.5 - PDF Reader с активными ссылками")
        ...
        self.debug_mode = False  # Режим отладки ссылок
    ...
    def setup_ui(self):
        ...
        # Новая кнопка для отладки ссылок
        self.action_debug_links = QAction(create_text_icon("🔗"), "Отладка ссылок", self)
        self.action_debug_links.triggered.connect(self.toggle_debug_links)
        self.action_debug_links.setCheckable(True)
        self.action_debug_links.setChecked(False)
        toolbar.addAction(self.action_debug_links)
    ...
    def disable_controls(self):
        controls = [
            ...
            self.action_search, self.action_debug_links
        ]
    ...
    def toggle_debug_links(self):
        """Включает/выключает режим отладки ссылок"""
        self.debug_mode = self.action_debug_links.isChecked()
        if self.document:
            self.render_page()  # Перерисовываем, чтобы показать/скрыть ссылки
        if self.debug_mode:
            self.status_bar.showMessage("Режим отладки ссылок включен - ссылки подсвечиваются")
        else:
            self.status_bar.showMessage("Режим отладки ссылок выключен")
    ...
    def open_file(self, file_path=None):
        ...
        if file_path:
            try:
                ...
                self.setWindowTitle(f"RuundPDF v2.9.5 - {QFileInfo(file_path).fileName()} (активные ссылки)")
        ...
        try:
            ...
            for link in links:
                if 'uri' in link:  # Это внешняя ссылка
                    ...
                    if self.debug_mode:
                        print(f"Найдена ссылка: {link['uri']} в прямоугольнике {rect}")
    ...
    def show_about_dialog(self):
        ...
        info_text.setHtml("""
            <p align='center'><strong>RuundPDF v2.9.5</strong></p>
            ...
            <ul>
                ...
                <li>✅ Режим отладки ссылок (подсветка активных областей)</li>
            ...
            <ul>
                ...
                <li>Включите "Отладка ссылок" для подсветки всех активных областей</li>
            </ul>
        """)
