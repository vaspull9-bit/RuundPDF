...
class TTSController(QObject):
    ...
    def _setup_voice(self, tts_engine):
        """Настраивает голос TTS - ИСПРАВЛЕННАЯ ВЕРСИЯ"""
        ...
        try:
            ...
            if voices:
                ...
                # Ищем женский голос
                if use_female:
                    print("Поиск женского голоса...")
                    female_indices = []
                    for i, voice in enumerate(voices):
                        voice_lower = voice.name.lower()
                        # Более широкий поиск женских голосов
                        if any(keyword in voice_lower for keyword in [
                            'female', 'женск', 'женский', 'женск', 
                            'alena', 'elena', 'irina', 'anna', 'maria',
                            'zira', 'eva', 'hazel', 'heera'
                        ]):
                            female_indices.append(i)
                            print(f"  Найден женский голос: {voice.name}")
                    
                    if female_indices:
                        # Выбираем первый найденный женский голос
                        tts_engine.setProperty('voice', voices[female_indices[0]].id)
                        print(f"Выбран женский голос: {voices[female_indices[0]].name}")
                        return
                    
                    print("Женский голос не найден, используется первый доступный")
                
                # Ищем мужской голос (по умолчанию)
                else:
                    print("Поиск мужского голоса...")
                    male_indices = []
                    for i, voice in enumerate(voices):
                        voice_lower = voice.name.lower()
                        # Более широкий поиск мужских голосов
                        if any(keyword in voice_lower for keyword in [
                            'male', 'мужск', 'мужской', 'мужск',
                            'david', 'mark', 'ivan', 'pavel', 'alex',
                            'harry', 'george', 'mikhail', 'сергей',
                            'михаил', 'alexey'
                        ]):
                            male_indices.append(i)
                            print(f"  Найден мужской голос: {voice.name}")
                    
                    if male_indices:
                        # Выбираем первый найденный мужской голос
                        tts_engine.setProperty('voice', voices[male_indices[0]].id)
                        print(f"Выбран мужской голос: {voices[male_indices[0]].name}")
                        return
                    
                    print("Мужской голос не найден, используется первый доступный")
                
                # Если не нашли нужный голос - используем первый доступный
                tts_engine.setProperty('voice', voices[0].id)
                print(f"Используется голос по умолчанию: {voices[0].name}")
                
        except Exception as e:
            print(f"Ошибка настройки голоса: {e}")
            # Если что-то пошло не так, пробуем инициализировать без настроек
