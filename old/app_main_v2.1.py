"""
RuundPDF v2.4.4 - PDF Reader with TTS and advanced features
...
def resource_path(relative_path):
    ...
    так и в собранном исполняемом файле (PyInstaller).
...
# ============================================================================
# 2. ГЛОБАЛЬНАЯ ФУНКЦИЯ СОЗДАНИЯ ИКОНОК ИЗ ТЕКСТА
# ============================================================================
def create_text_icon(text, size=32):
    ...
    Args:
        text: Символ Unicode для отображения
...
def register_file_association():
    ...
    try:
        ...
        # Устанавливаем команду открытия
...
class AboutDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        info_text.setHtml("""
            <p align='center'><strong>Программа RuundPDF v2.4.4</strong></p>
...
class TTSConfigDialog(QDialog):
    """
    Диалоговое окно настроек плеера озвучки.
    """
...
class TTSPlayerWidget(QWidget):
    """
    Виджет плеера для озвучки текста.
    """
    ...
    def set_read_mode(self, mode, page=0):
        ...
        Args:
            mode: Режим чтения ('current', 'start', 'specific', 'only_current')
            page: Номер страницы для режима 'specific'
        """
        self.read_mode = mode
        self.specific_page = page
    
    def go_to_first_page(self):
        """Переходит на первую страницу и останавливает чтение."""
        self.current_read_page = 0
        if self.is_playing:
            self.stop_playback()
    
    def go_to_prev_page(self):
        """Переходит на предыдущую страницу и останавливает чтение."""
    ...
    def go_to_next_page(self):
        """Переходит на следующую страницу и останавливает чтение."""
    ...
    def go_to_last_page(self):
        """Переходит на последнюю страницу и останавливает чтение."""
    ...
    def start_playback(self):
        """Начинает воспроизведение с учетом выбранного режима."""
        # Определяем СТАРТОВУЮ страницу для каждого режима
        if self.read_mode == 'start':
            start_page = 0
        elif self.read_mode == 'specific':
            start_page = self.specific_page
        elif self.read_mode == 'only_current':
            start_page = self.current_read_page  # Только текущая
        else:  # 'current' - по умолчанию
            start_page = self.current_read_page
        
        # Сохраняем стартовую страницу для чтения
    ...
    def stop_playback(self):
        ...
        # Безопасная остановка движка TTS
        if self.tts_engine:
            try:
    ...
    def playback_worker(self):
        """Рабочий поток для воспроизведения с правильной логикой."""
        try:
            # ОПРЕДЕЛЯЕМ ДИАПАЗОН СТРАНИЦ для чтения
            if self.read_mode == 'only_current':
                # Режим 1: ТОЛЬКО текущая страница
                pages_to_read = [self.current_read_page]
            else:
                # Режимы 2-4: Читаем ВЕСЬ ФАЙЛ с нужной стартовой страницы
                if self.read_mode == 'start':
                    start_page = 0
                elif self.read_mode == 'specific':
                    start_page = self.specific_page
                ...
                pages_to_read = list(range(start_page, self.total_pages))
            ...
            for page_num in pages_to_read:
                ...
                # Получаем и озвучиваем текст страницы
...
class PDFGraphicsView(QGraphicsView):
    """
    Кастомный QGraphicsView для отображения PDF с поддержкой выделения текста.
    """
    ...
    def wheelEvent(self, event):
        ...
        С Ctrl - зум, без Ctrl - перелистывание страниц.
        """
    ...
    def paintEvent(self, event):
        ...
        Добавляет отрисовку выделения текста.
        """
...
class PDFViewerApp(QMainWindow):
    """
    Основной класс приложения PDF-ридера.
    """
    def __init__(self, file_to_open=None):
        ...
        def get_icon_path():
            ...
            if getattr(sys, 'frozen', False):
                ...
                base_path = os.path.abspath(".")
            icon_full_path = os.path.join(base_path, 'icon.png')
    ...
    def disable_controls(self):
        """Отключает элементы управления при отсутствии документа."""
    ...
    def enable_controls(self):
        """Включает элементы управления при загрузке документа."""
        controls = [self.action_prev, self.action_next, self.action_rotate_left, 
                    self.action_rotate_right, self.action_speak, self.zoom_slider,
                    self.action_save, self.action_save_as, self.action_print, 
                    self.action_add_bookmark, self.action_bookmark, self.action_toggle_cursor]
    ...
    def extract_text_with_rectangles(self, page):
        ...
        Returns:
            list: Список словарей с текстом и координатами
        """
        text_blocks = []
    ...
    def get_text_in_rectangle(self, selection_rect):
        ...
        Args:
            selection_rect: Прямоугольник выделения
            
        Returns:
            str: Текст внутри прямоугольника
        """
    ...
    def rotate_left(self):
        """Поворачивает страницу на 90° влево."""
        self.rotation_angle = (self.rotation_angle - 90) % 360
        if self.document:
            self.render_page()
    
    def rotate_right(self):
        """Поворачивает страницу на 90° вправо."""
        self.rotation_angle = (self.rotation_angle + 90) % 360
        if self.document:
            self.render_page()
    ...
    def get_text_for_page(self, page_num):
        """
        Получает текст указанной страницы.
    ...
    def show_tts_player(self):
        """Показывает окно плеера озвучки."""
        if not self.document:
    ...
    def add_bookmark(self):
        """Добавляет закладку на текущую страницу."""
        if not self.document:
            return
    ...
    def manage_bookmarks(self):
        """Открывает диалог управления закладками."""
        if not self.bookmarks:
    ...
    def delete_bookmark_dialog(self, parent_dialog):
        ...
        Args:
            parent_dialog: Родительское диалоговое окно
        """
        if not self.bookmarks:
            return
    ...
    def view_mouse_press_event(self, event):
        """
        Обработчик нажатия кнопки мыши в режиме выделения текста.
    ...
    def view_mouse_move_event(self, event):
        """
        Обработчик движения мыши в режиме выделения текста.
    ...
    def view_mouse_release_event(self, event):
        """
        Обработчик отпускания кнопки мыши в режиме выделения текста.
