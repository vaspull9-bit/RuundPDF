"""
RuundPDF v2.8.0 - PDF Reader с РАБОЧЕЙ озвучкой и поиском
...
# ============================================================================
# КЛАСС ДЛЯ УПРАВЛЕНИЯ ОЗВУЧКОЙ (ИСПРАВЛЕННЫЙ)
# ============================================================================
class TTSController(QObject):
    """Контроллер для управления озвучкой"""
    ...
    def stop_reading(self):
        ...
        if self.is_running:
            ...
            self.is_paused = False
            self.stop_event.set()
            self.pause_event.set()  # Разблокируем поток если он на паузе
        ...
        try:
            ...
            for page_num in range(self.current_page, self.end_page + 1):
                ...
                if self.pause_event.is_set():
                    self.pause_event.wait()  # Блокируем поток до снятия паузы
                ...
                # Озвучиваем текст С СОЗДАНИЕМ НОВОГО ДВИЖКА для каждой страницы
                try:
                    # Создаем новый движок для каждой страницы
                    tts_engine = pyttsx3.init()
                    self._setup_voice(tts_engine)
...
# ============================================================================
# КЛАСС ДЛЯ ПОИСКА ТЕКСТА (ИСПРАВЛЕННЫЙ)
# ============================================================================
...
# ============================================================================
# ОСНОВНОЙ КЛАСС ПРИЛОЖЕНИЯ (ИСПРАВЛЕННЫЙ)
# ============================================================================
class PDFViewerApp(QMainWindow):
    ...
    def highlight_search_result(self, rect, text):
        """Подсвечивает найденный текст на странице - ИСПРАВЛЕННАЯ ВЕРСИЯ"""
        # Очищаем предыдущие подсветки
        for item in self.search_highlights:
            self.scene.removeItem(item)
        self.search_highlights.clear()
        
        # Проверяем что rect содержит координаты
        if hasattr(rect, 'x0') and hasattr(rect, 'y0') and hasattr(rect, 'x1') and hasattr(rect, 'y1'):
            # Создаем подсветку для найденного текста
            ...
            # ИСПРАВЛЕННАЯ СТРОКА: используем QPen(Qt.PenStyle.NoPen)
            highlight.setPen(QPen(Qt.PenStyle.NoPen))
            highlight.setBrush(QBrush(QColor(255, 255, 0, 100)))  # Желтый полупрозрачный
            ...
            print(f"Ошибка: rect не содержит ожидаемых атрибутов: {rect}")
    ...
    def render_page(self):
        ...
        # Восстанавливаем подсветку поиска для текущей страницы
    ...
    def get_text_for_page(self, page_num):
        """Функция для получения текста страницы (используется плеером)"""
        if self.document and 0 <= page_num < self.document.page_count:
            page = self.document.load_page(page_num)
            text = page.get_text()
            # Возвращаем текст или пробел если страница пустая
    ...
    def show_about_dialog(self):
        ...
        info_text.setHtml("""
            <p align='center'><strong>RuundPDF v2.8.0</strong></p>
            <p align='center'>© DeeR Tuund 2025</p>
            <p>PDF ридер с РАБОЧЕЙ озвучкой и поиском</p>
            <p>Функции:</p>
            <ul>
