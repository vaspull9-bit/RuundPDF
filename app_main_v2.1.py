...
from PyQt6.QtGui import (QPixmap, QImage, QIcon, QAction, QPainter, 
                         QPageLayout, QPageSize, QDropEvent, QDragEnterEvent, QFont) # QFont добавлен здесь
from PyQt6.QtCore import Qt, QSize, QFileInfo, QSettings, QTimer
from PyQt6.QtPrintSupport import QPrintDialog

# --- 2.1. ГЛОБАЛЬНАЯ ФУНКЦИЯ СОЗДАНИЯ ИКОНОК ИЗ ТЕКСТА (ВОССТАНОВЛЕНО) ---
...
class AboutDialog(QDialog):
    def __init__(self, parent=None):
        ...
        info_text.setHtml("""
            <p align='center'><strong>Программа RuundPDF v2.1.0</strong></p>
...
class TTSConfigDialog(QDialog):
    ...
    def initUI(self):
        ...
        if self.player.read_mode == 'start': self.radio_start.setChecked(True)
        elif self.player.read_mode == 'specific': self.radio_specific.setChecked(True)
        elif self.player.read_mode == 'only_current': self.radio_only_current.setChecked(True)
        else: self.radio_current.setChecked(True)

    def load_settings(self):
        use_male = self.player.settings.value("tts_voice_m", True, type=bool)
        use_female = self.player.settings.value("tts_voice_f", False, type=bool)
        if use_female: self.radio_voice_f.setChecked(True)
        else: self.radio_voice_m.setChecked(True)
    ...
    def _run_tts_loop(self):
        try:
            if self.read_mode == 'only_current':
                text = self.text_provider(self.current_read_page)
                if text:
...
class PDFViewerApp(QMainWindow):
    ...
    def disable_controls(self):
        controls = [self.action_prev, self.action_next, self.action_rotate_left, 
                    ...
                    self.action_save, self.action_save_as, self.action_print, self.action_add_bookmark]
    ...
    def save_file(self):
        if self.document and self.file_path:
            try:
                # Используем incremental=True для сохранения изменений в существующий файл
                self.document.save(self.file_path, incremental=True, garbage=4, deflate=True)
                QMessageBox.information(self, "Успех", "Файл успешно сохранен.")
            except ValueError as e:
                # Если файл только для чтения или другая проблема
                QMessageBox.critical(self, "Ошибка сохранения", f"Не удалось сохранить изменения в исходный файл: {e}")
                self.save_file_as() # Предлагаем сохранить как новый файл
        elif self.document:
             self.save_file_as()
